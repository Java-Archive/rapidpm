...
(function () {
    ...
    function xa(a, b, c) {
        var d, e;
        if (Ib(b))if (L(c))a.setAttribute(b, c); else {
            if (a && a.getAttribute)e = a.getAttribute(b)
        } else if (L(b) && Jb(b))for (d in b)a.setAttribute(d,
            b[d]);
        return e
    }
    ...
    function La(a, b) {
        if (Ac)if (b && b.opacity !== Qa)b.filter = "alpha(opacity=" + b.opacity * 100 + ")";
        ma(a.style, b)
    }
    ...
    function Hd(a, b) {
        function c(m, h) {
                    ...
                    if (r) {
                        ...
                        if (o.isCartesian) {
                            t(o.data, function (ja) {
                                ...
                                S = S ? fa : C;
                                if (Sa === null)Sa = v = ja[I];
                                if (ka)if (D > v)v = D; else {
                                    if (D < Sa)Sa = D
                                } else if (L(la)) {
                                    if (U)Z[D] = L(Z[D]) ? Z[D] + la : la;
                                    la = Z ? Z[D] : la;
                                    ja = y(ja.low, la);
                                    if (!Ic)if (la > v)v = la; else if (ja < Sa)Sa = ja;
                                    if (U) {
                                        ca[S] || (ca[S] = {});
                            ...
                            if (/(area|column|bar)/.test(o.type) && !ka)if (Sa >= 0) {
                                Sa = 0;
                                Id = true
                            } else if (v < 0) {
                                v = 0;
            ...
            function ga(l, p) {
                ...
                if (!p) {
                    p = [1, 2, 2.5, 5, 10];
                    if (h.allowDecimals === false)if (Db == 1)p = [1, 2, 5, 10]; else if (Db <= 0.1)p = [1 / Db]
                }
            ...
            function da() {
                ...
                if (!bb && !Ic && !Nb && L(J) && L(Q)) {
                    l = Q - J || 1;
                    if (!L(h.min) && !L(na) && Wb && (Sa < 0 || !Id))J -= l * Wb;
                    if (!L(h.max) && !L(Ma) && Kd && (v > 0 || !Jd))Q += l * Kd
                }
                Ua = J == Q ? 1 : Nb && !U && X == r.options.tickPixelInterval ? r.tickInterval : y(U, bb ? 1 : (Q - J) * X / B);
                if (!N && !L(h.tickInterval))Ua = ga(Ua);
                s.tickInterval = Ua;
                ...
                if (N) {
                    ...
                    for (r = 0; r < D.length; r++) {
                        ...
                        S = la[1];
                        Z = la[2];
                        if (D[r + 1])if (Ua <= (S * Z[Z.length - 1] + D[r + 1][1]) / 2)break
                    }
                    if (S == ja && Ua < 5 * S)Z = [1, 2, 5];
                    D = ga(Ua / S, Z);
                    Z = new Date(J * ob);
                    Z.setMilliseconds(0);
                    if (S >= H)Z.setSeconds(S >= C ? 0 : D * Mb(Z.getSeconds() /
                        D));
                    if (S >= C)Z[Cd](S >= fa ? 0 : D * Mb(Z[$c]() / D));
                    if (S >= fa)Z[Dd](S >= X ? 0 : D * Mb(Z[ad]() / D));
                    if (S >= X)Z[cd](S >= o ? 1 : D * Mb(Z[oc]() / D));
                    if (S >= o) {
                    ...
                    for (fa = Z[oc](); H < Q && r < Ca;) {
                        pa.push(H);
                        if (S == ja)H = Cc(p + r * D, 0) / ob; else if (S == o)H = Cc(p, C + r * D) / ob; else if (!U && (S == X || S == l))H = Cc(p, C, fa + r * D * (S == X ? 1 : 7)); else H += S * D;
                        r++
                    }
                    pa.push(H);
                ...
                if (!Nb) {
                    if (bb || ka && m.hasColumn) {
                        p = (bb ? 1 : Ua) * 0.5;
                        if (bb || !L(y(h.min, na)))J -= p;
                        if (bb || !L(y(h.max, Ma)))Q += p
                    }
                    p = pa[0];
                    r = pa[pa.length - 1];
                    if (h.startOnTick)J = p; else J > p && pa.shift();
                    if (h.endOnTick)Q = r; else Q < r && pa.pop();
                    Eb || (Eb = {x:0, y:0});
                    if (!N && pa.length > Eb[I])Eb[I] = pa.length
                }
            }
            ...
            function Da() {
                ...
                ha = F;
                F = B / (Q - J || 1);
                if (!ka)for (l in ca)for (p in ca[l])ca[l][p].cum = ca[l][p].total;
                if (!s.isDirty)s.isDirty = J != fb || Q != cc
            }
            ...
            function Ya() {
                ...
                va = O ? W : pb;
                if (o || Nb) {
                    if (Jc && !bb)for (o = J + (pa[0] - J) % Jc; o <= Q; o += Jc) {
                ...
                t([qb, Xb, dc], function (C) {
                    for (var fa in C)if (C[fa].isActive)C[fa].isActive = false; else {
                        C[fa].destroy();
                        delete C[fa]
                    }
                });
                if (r) {
                    ...
                    if (Ea)Ea.animate({d:o}); else Ea = $.path(o).attr({stroke:h.lineColor,
            ...
            x.prototype = {addLabel:function () {
                ...
                if (U === Qa)this.label = L(l) && r && p.enabled ? $.text(l, 0, 0).attr({align:p.align, rotation:p.rotation}).css(ma(o, p.style)).add(rb) : null; else U && U.attr({text:l}).css(o)
                ...
                if (S) {
                    ...
                    if (D)D.animate({d:C}); else this.mark = $.path(C).attr({stroke:Z, "stroke-width":S}).add(rb)
                }
                if (o) {
            ...
            w.prototype = {render:function () {
                ...
                if (U) {
                    ...
                    if (fa)p.dashstyle = fa
                } else if (L(C) && L(X)) {
                    C = Ga(C, J);
                    X = nb(X, Q);
                    H = Ia(X);
                    if ((D = Ia(C)) && H)D.push(H[4], H[5], H[1], H[2]); else D = null;
                    p =
                    {fill:Z}
                } else return;
                if (L(S))p.zIndex = S;
                if (ja)if (D)ja.animate({d:D}, null, ja.onGetPath); else {
                    ja.hide();
                ...
                if (r && L(r.text) && D && D.length && Ca > 0 && ra > 0) {
                    ...
                    if (!o)l.label = o = $.text(r.text, 0, 0).attr({align:r.textAlign || r.align,
            ...
            Ia = function (l, p, r) {
                ...
                r = U = V(l + va);
                o = X = V(H - l - va);
                if (isNaN(l))fa = true; else if (O) {
                    o = aa;
                    X = H - pb;
                    if (r < W || r > W + Ca)fa = true
                } else {
                    r = W;
                    U = C - zb;
                    if (o < aa || o > aa + ra)fa = true
                }
                return fa ? null : $.crispLine([Wa, r, o, Ba, U, X], p || 0)
            };
            if (Fa && ka && Yb === Qa)Yb = true;
            ma(s, {addPlotBand:sa, addPlotLine:sa, adjustTickAmount:function () {
                if (Eb && !N && !bb && !Nb) {
                    var l = ec, p = pa.length;
                    ec = Eb[I];
                    if (p < ec) {
                        for (; pa.length < ec;)pa.push(M(pa[pa.length - 1] + Ua));
                        F *= (p - 1) / (ec - 1);
                        Q = pa[pa.length - 1]
                    }
                    if (L(l) &&
                ...
                if (l || Nb) {
                    t(pa, function (C) {
                        if (qb[C])qb[C].addLabel(); else qb[C] = new x(C);
                        if (G === 0 || G ==
                            2 || {1:"left", 3:"right"}[G] == U.align)fc = Ga(qb[C].getLabelSize(), fc)
                    });
                    if (Nc)fc += (Nc - 1) * 16
                } else for (var H in qb) {
                    qb[H].destroy();
                    delete qb[H]
                }
                if (o && o.text) {
                    if (!s.axisTitle)s.axisTitle = $.text(o.text, 0, 0).attr({zIndex:7, rotation:o.rotation || 0, align:o.textAlign || {low:"left", middle:"center", high:"right"}[o.align]}).css(o.style).add();
        ...
        function e(m) {
            ...
            return{shared:sa, refresh:function (I) {
                ...
                if (ta === false || !ha)w(); else {
                    ...
                    if (F < 5)F = 5; else if (F + O > Oa)F = Oa - O - 5;
                    x(V(B - Ha),
                        V(F - Ha))
                }
                if (M) {
                    M = nc(M);
                    F = M.length;
                    for (var Fb; F--;)if (M[F] && (Fb = I.series[F ? "yAxis" : "xAxis"])) {
                        B = Fb.getPlotLinePath(I[F ? "y" : "x"], 1);
                        if (da[F])da[F].attr({d:B, visibility:Ab}); else {
                            ha = {"stroke-width":M[F].width || 1, stroke:M[F].color || "#C0C0C0", zIndex:2};
                            if (M[F].dashStyle)ha.dashstyle = M[F].dashStyle;
        ...
        function f(m, h) {
            function x(G) {
                var ca;
                G = G || hb.event;
                if (!G.target)G.target = G.srcElement;
            ...
            Pc = function () {
                ...
                } else m.trackerGroup = Qc = $.g("tracker").attr({zIndex:9}).add()
            };
            Pc();
            ...
            (function () {
                ...
                var ca = function (s) {
                    if (!(s && s.touches && s.touches.length > 1)) {
                        ...
                        if (Gb && s.type == "touchstart")if (xa(s.target, "isTracker"))m.runTrackerClick || s.preventDefault(); else!$d && !I && s.preventDefault();
                        if (I) {
                            G || P();
                            if (N < W)N = W; else if (N > W + Ca)N = W + Ca;
                            if (R < aa)R = aa; else if (R > aa + ra)R = aa + ra
                        }
                        if (ld && s.type != "touchstart") {
                            if (Da = Math.sqrt(Math.pow(M - N, 2) + Math.pow(da - R, 2)) > 10) {
                                if (ic && (Ha || ka) && hc(M - W, da - aa))sa || (sa = $.rect(W, aa, Na ? 1 : Ca, O ? 1 : ra, 0).attr({fill:"rgba(69,114,167,0.25)", zIndex:7}).add());
                                if (sa && Na) {
                                    N = N - M;
                            ...
                            if (rc && h.shared) {
                                B = [];
                                F = Aa.length;
                                for (ha = 0; ha < F; ha++)if (Aa[ha].visible && Aa[ha].tooltipPoints.length) {
                                    s = Aa[ha].tooltipPoints[ta];
                                    s._dist = ab(ta - s.plotX);
                                    va = nb(va, s._dist);
                                    B.push(s)
                                }
                                for (F = B.length; F--;)B[F]._dist > va && B.splice(F, 1);
                                if (B.length && B[0].plotX != jd) {
                                    rc.refresh(B);
                                    jd = B[0].plotX
                                }
                            }
                            if (N && N.tracker)(s = N.tooltipPoints[ta]) && s != R && s.onMouseOver()
                        }
                        return(G =
                            I) || !ic
                    }
                ...
                ua.onclick = function (s) {
                    ...
                    s = x(s);
                    s.cancelBubble = true;
                    if (!Da)if (N && xa(s.target, "isTracker")) {
        ...
        function g(m) {
            ...
            if (w)if (Fa && h == "column")x = sb.bar; else if (!Fa && h == "bar")x = sb.column;
            h = new x;
            h.init(q, m);
            if (!w && h.inverted)Fa = true;
        ...
        function j(m) {
            ...
            t(Aa, function (da) {
                if (da.isDirty) {
                    da.cleanData();
                    da.getSegments();
                    if (da.options.legendType == "point")h = true
                }
            });
        ...
        function n(m, h) {
            ...
            t([
                ...
                if (x && x.text && !P)q[w] = $.text(x.text, 0, 0).attr({align:x.align, "class":"highcharts-" + w, zIndex:1}).css(x.style).add().align(x, false, uc)
            })
        }
        ...
        function z() {
            ib = u.renderTo;
            Od = $b + od++;
            if (Ib(ib))ib = za.getElementById(ib);
            ib.innerHTML = "";
            if (!ib.offsetWidth) {
                Rb = ib.cloneNode(0);
        ...
        function ia() {
            ...
            if (!q.seriesGroup)q.seriesGroup = $.g("series-group").attr({zIndex:3}).add();
            t(Aa, function (w) {
                w.translate();
                w.setTooltipPoints();
                w.render()
            ...
            if (h.enabled && !q.credits) {
                x = h.href;
                $.text(h.text, 0, 0).on("click",function () {
        ...
        function K() {
            if (!xc && !hb.parent && za.readyState != "complete")za.attachEvent("onreadystatechange", function () {
        ...
        var ba = y(u.marginTop, A[0]), ya = y(u.marginRight, A[1]), ea = y(u.marginBottom, A[2]), qa = y(u.marginLeft, A[3]), $a = u.spacingTop, jb = u.spacingRight, sd = u.spacingBottom, Vc = u.spacingLeft, uc, kc, tc, aa, zb, pb, W, Qb, ib, Rb, ua, Od, Tc, vc, Va, Oa, hd, Oc, td, ud, vd, wd, q = this, $d = (A = u.events) && !!A.click, xd, hc, rc, ld, ac, Rd, yd, ra, Ca, gc, Qc, Pc, md, Sb, Tb, qc, ic =
            ...
            function P(v) {
                ...
                if (!fb) {
                    ...
                    v.legendItem = fb = $.text(M.labelFormatter.call(v), 0, 0).css(v.visible ? ka : O).on("mouseover",function () {
                        v.setState(xb);
                        fb.css(Na)
                    ...
                    if (!J && v.options && v.options.lineWidth) {
                        ...
                        if (cc.dashStyle)Q.dashstyle = cc.dashStyle;
                        v.legendLine = $.path([Wa, -Da - sa, 0, Ba, -sa, 0]).attr(Q).add(Ia)
                    }
                    if (J)Y = $.rect(na = -Da - sa, Ma = -11, Da, 12, 2).attr({"stroke-width":0, zIndex:3}).add(Ia); else if (v.options && v.options.marker && v.options.marker.enabled)Y = $.symbol(v.symbol, na = -Da / 2 - sa, Ma = -4, v.options.marker.radius).attr(v.pointAttr[cb]).attr({zIndex:3}).add(Ia);
                    if (Y) {
                        Y.xOff = na;
                        Y.yOff = Ma
                    }
            ...
            function ga() {
                ...
                Sa && Ea.reverse();
                t(Ea, function (Ma) {
                    if (Ma.options.showInLegend)t(Ma.options.legendType == "point" ? Ma.data : [Ma], P)
                });
                Sa && Ea.reverse();
                Sb = Fb || rb;
                Tb = B - s + F;
                if (va || ta) {
                    Sb += 2 * G;
                    Tb += 2 * G;
                    if (ha)Sb > 0 && Tb > 0 && ha.animate({width:Sb, height:Tb}); else ha = $.rect(0, 0, Sb, Tb, M.borderRadius, va || 0).attr({stroke:M.borderColor, "stroke-width":va || 0, fill:ta || mb}).add(Ia).shadow(M.shadow);
        ...
        kd = function (m) {
            ...
            !m || m.resetSelection ? t(Xa, function (w) {
                ...
                function (w) {
                    var P = w.axis;
                    if (q.tracker[P.isXAxis ? "zoomX" : "zoomY"])P.setExtremes(w.min, w.max, false, x)
                });
            j()
        };
        sc = function () {
            ...
            qd();
            if ((q.title || q.subtitle) && !L(ba))if (M = Ga(q.title && !kc.floating && !kc.verticalAlign && kc.y || 0, q.subtitle && !tc.floating && !tc.verticalAlign && tc.y || 0))aa = Ga(aa, M + y(kc.margin, 15) + $a);
            if (m.enabled && !m.floating)if (P == "right")L(ya) || (zb = Ga(zb, Sb - x + h + jb)); else if (P == "left")L(qa) || (W = Ga(W, Sb + x + h + Vc)); else if (ga ==
                "top")L(ba) || (aa = Ga(aa, Tb + w + h + $a)); else if (ga == "bottom")L(ea) || (pb = Ga(pb, Tb - w + h + sd));
            ic && t(Xa, function (da) {
                da.getOffset()
            });
            L(qa) || (W += Qb[3]);
        ...
        nd = function () {
            ...
            P = m + (u.shadow ? 8 : 0);
            if (m || h)if (td)td.animate({width:Va - P, height:Oa - P}); else td = $.rect(P / 2, P / 2, Va - P, Oa - P, u.borderRadius, m).attr({stroke:u.borderColor, "stroke-width":m, fill:h || mb}).add().shadow(u.shadow);
            if (x)if (ud)ud.animate(ga); else ud = $.rect(W, aa, Ca, ra, 0).attr({fill:x}).add().shadow(u.plotShadow);
            if (w)if (vd)vd.animate(ga); else vd = $.image(w, W, aa, Ca, ra).add();
            if (u.plotBorderWidth)if (wd)wd.animate(ga); else wd = $.rect(W, aa, Ca, ra, 0, u.plotBorderWidth).attr({stroke:u.plotBorderColor,
        ...
        q.get = function (m) {
            ...
            for (h = 0; h < Aa.length; h++)if (Aa[h].options.id ==
                m)return Aa[h];
    ...
    var Vb = function (a) {
        var b =
            [], c;
        (function (d) {
            if (c = /rgba\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]?(?:\.[0-9]+)?)\s*\)/.exec(d))b = [oa(c[1]), oa(c[2]), oa(c[3]), parseFloat(c[4], 10)]; else if (c = /#([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})/.exec(d))b = [oa(c[1], 16), oa(c[2], 16), oa(c[3], 16), 1]
        })(a);
    ...
    Mc = function (a, b, c) {
        ...
        if (!L(b) || isNaN(b))return"Invalid date";
        a = y(a, "%Y-%m-%d %H:%M:%S");
        b = new Date(b * ob);
    ...
    Hc.prototype = {init:function (a, b) {
        ...
        if (Ib(a)) {
            ...
            E = xa(g, c) || this[c] || 0;
            if (c != "d" && c != "visibility")E = parseFloat(E)
        } else for (c in a) {
            k = false;
            d = a[c];
            if (c == "d") {
                ...
                if (this.rotation)xa(g, "transform", "rotate(" + this.rotation + " " + d + " " + oa(a.y || xa(g, "y")) + ")")
            } else if (c == "fill")d = j.color(d, g,
                c); else if (i == "circle" && (c == "x" || c == "y"))c = {x:"cx", y:"cy"}[c] || c; else if (c == "translateX" || c == "translateY" || c == "rotation" || c == "verticalAlign") {
                this[c] = d;
            ...
            if (be && c == "stroke-width" && d === 0)d = 1.0E-6;
            ...
            if (n && /^(width|height|visibility|x|y|d)$/.test(c))for (e = n.length; e--;)xa(n[e], c, d);
            if (c == "text") {
                this.textStr =
                    d;
                j.buildText(this)
        ...
        var fe = wb(Hc, {init:function (a, b) {
            ...
            if (Ib(a)) {
                ...
                if (k && /^(x|y|r|start|end|width|height|innerR)/.test(c)) {
                    ...
                    this.dashstyle = d;
                    n = true
                } else if (c == "fill")if (i == "SPAN")g.color = d; else {
                ...
                if (!n)if (yc)f[c] = d; else xa(f, c, d)
            }
            return ia
        }, clip:function (a) {
        ...
        setData:function (a, b) {
            ...
            if (this.options.marker.enabled)for (f = b.length; f--;) {
                ...
                if (e !== Qa && !isNaN(e)) {
                    ...
                    i = a.r;
                    if (j)j.animate({x:d, y:e, r:i}); else g.graphic = c.renderer.symbol(y(g.marker && g.marker.symbol, this.symbol), d, e, i).attr(a).add(this.group)
                }
            }
            ...
            t(a.segments, function (u) {
                n = [];
                t(u, function (ea, qa) {
                    if (a.getPointSpline)n.push.apply(n, a.getPointSpline(u, ea, qa)); else {
                        n.push(qa ? Ba : Wa);
                        qa && b.step && n.push(ea.plotX, u[qa - 1].plotY);
                        n.push(ea.plotX, ea.plotY)
                    }
                });
                if (u.length > 1)d = d.concat(n); else T.push(u[0]);
                if (ia) {
                    var A = [], ba, ya = n.length;
                    for (ba = 0; ba < ya; ba++)A.push(n[ba]);
                    ya == 3 && A.push(Ba, n[1], n[2]);
                    if (b.stacking && a.type != "areaspline")for (ba = u.length - 1; ba >= 0; ba--)A.push(u[ba].plotX, u[ba].yBottom); else A.push(Ba, u[u.length - 1].plotX, E, Ba, u[0].plotX, E);
                    K = K.concat(A)
                }
            });
            a.graphPath = d;
            ...
            if (ia) {
                ...
                if (f)f.animate({d:K}); else a.area = a.chart.renderer.path(K).attr({fill:e}).add(g)
            }
            ...
            if (g)g.attr({d:c}); else a.tracker = e.renderer.path(c).attr({isTracker:true, stroke:Ud, fill:mb, "stroke-width":b.lineWidth + 2 * f, visibility:a.visible ? Ab : tb, zIndex:1}).on(Gb ? "touchstart" : "mouseover",function () {
    ...
    Ka = wb(lb, {type:"pie", isCartesian:false, pointClass:Ka, pointAttrToOptions:{stroke:"borderColor", "stroke-width":"borderWidth",
        ...
        t(this.data, function (g) {
            ...
            e.translate(c[0], c[1]);
            if (d)d.animate(f); else g.graphic = b.arc(f).attr(ma(g.pointAttr[cb], {"stroke-linejoin":"round"})).add(g.group);
                ...
                for (qa = 0; qa < E[ea].length; qa++) {
                    ...
                    if (g = b.dataLabel) {
                        ...
                        if (j)if (u && b.rank < a)K = tb; else if (!ba &&
                            ...
                            T = n + A * k;
                            ia = this.getX(T, ea > 1);
                            if (!ba && T + k > z || ba && T - k < z)if (u)K = tb; else a++
                        }
