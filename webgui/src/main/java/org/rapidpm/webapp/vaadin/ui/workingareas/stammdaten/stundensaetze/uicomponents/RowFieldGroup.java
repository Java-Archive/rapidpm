...
public class RowFieldGroup
    extends Binder<RessourceGroup> {
  ...
  private void buildForm() {
    ResourceBundle messages = VaadinSession.getCurrent()
                                           .getAttribute(ResourceBundle.class);
    forField(name).withNullRepresentation("")
    ...
    forField(hoursPerWeek).withNullRepresentation("")
    ...
    forField(weeksPerYear).withNullRepresentation("")
                          .withConverter(new StringToIntegerConverter("Integer-Value!"))
                          .asRequired()
    ...
    forField(planAnzahl).withNullRepresentation("")
                        .withConverter(new StringToIntegerConverter("Integer-Value!"))
                        .asRequired()
    ...
    forField(facturizable).withNullRepresentation("")
                          .withConverter(Double::parseDouble, value -> value == null
                                                                       ? "0.0"
                                                                       : value.toString())
    ...
    forField(externalEurosPerHour).withNullRepresentation("")
                                  .withConverter(Double::parseDouble, value -> value == null
                                                                               ? "0.0"
                                                                               : value.toString())
                                  .withValidator(new DoubleRangeValidator("Double-Value!", 0.0d, 100.0d))
    ...
    forField(bruttoGehalt).withNullRepresentation("")
                          .withConverter(Double::parseDouble, value -> value == null
                                                                       ? "0.0"
                                                                       : value.toString())
                          .withValidator(new DoubleRangeValidator("Double-Value!", 0.0d, 100.0d))
  ...
  String getDefaultValueByField(TextField textField) {
    return defaultValueByField.get(textField)
                              .getDefaultValue();
  }
  ...
  List<TextField> getTextFields() {
    return new ArrayList<>(
        Arrays.asList(name, hoursPerWeek, weeksPerYear, planAnzahl, facturizable, externalEurosPerHour, bruttoGehalt));
  }
