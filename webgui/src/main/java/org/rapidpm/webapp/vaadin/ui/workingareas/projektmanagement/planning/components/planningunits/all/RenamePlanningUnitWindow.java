 ...
 */
public class RenamePlanningUnitWindow
    extends RapidWindow {
  ...
  public RenamePlanningUnitWindow(final ProjektplanungScreen screen) {
    final ResourceBundle messagesBundle = VaadinSession.getCurrent()
                                                       .getAttribute(ResourceBundle.class);
//        setText(messagesBundle.getString("planning_renamePlanningUnit"));
    fieldGroup             = new PlanningUnitFieldGroup(screen, screen.getPlanningUnitsTree()
                                                                      .getSelectedItems()
                                                                      .iterator()
                                                                      .next());
        ...
        saveButton.addClickListener((ComponentEventListener<ClickEvent<Button>>) buttonClickEvent -> {
          ...
          try {
            ...
            if (changedPlanningUnit.getId() == ProjektplanungScreen.PLATZHALTER_ID) {
              ...
              fieldGroup.getFieldForProperty(PlanningUnit.PARENT)
                        .ifPresent(abstractField -> abstractField.setRequiredIndicatorVisible(false));
              fieldGroup.writeBean(changedPlanningUnit);
...
//                            screen.getUi().setWorkingArea(new ProjektplanungScreen(screen.getUi()));
      ...
      public void setCancelButtonListener() {
        cancelButton.addClickListener(
            (ComponentEventListener<ClickEvent<Button>>) buttonClickEvent -> RenamePlanningUnitWindow.this.close());
      }
    };
    buildForm();
  }
  ...
  private void buildForm() {
    fieldGroup.getFieldForProperty(PlanningUnit.NAME)
              .ifPresent(this::add);
