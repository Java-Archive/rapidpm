...
public class CostsScreen
  ...
  public CostsScreen() {
    ...
    try {
      ...
      ressourceGroups = daoFactory.getRessourceGroupDAO()
                                  .loadAllEntities();
...
//            formLayout.add(upperFormLayout);
  ...
  private void fillOverviewGrid() {
    ...
    for (final RessourceGroup ressourceGroup : ressourceGroups) {
      Grid.Column<HashMap<String, String>> resourceGroupColumn = overviewGrid.addColumn(
          hashmap -> hashmap.get(ressourceGroup.getName()))
    ...
    for (Grid.Column<HashMap<String, String>> resourceGroupNameColumn : overviewGrid.getColumns()) {
      final Optional<RessourceGroup> matchingResourceGroup = ressourceGroups.stream()
                                                                            .filter(
                                                                                ressourceGroup -> ressourceGroup.getName()
                                                                                                                .equals(
                                                                                                                    resourceGroupNameColumn.getKey()))
                                                                            .findFirst();
      matchingResourceGroup.ifPresent(ressourceGroup -> overviewBean.put(ressourceGroup.getName(),
                                                                         ressourceGroup.getExternalEurosPerHour()
                                                                                       .toString()));
    }
    ...
    for (Grid.Column<HashMap<String, String>> resourceGroupNameColumn : overviewGrid.getColumns()) {
      ...
      matchingResourceGroup.ifPresent(ressourceGroup -> {
        ...
        final DaysHoursMinutesItem         itemForMinutesFromMap = new DaysHoursMinutesItem(minutesTotal,
    ...
    for (Grid.Column<HashMap<String, String>> resourceGroupNameColumn : overviewGrid.getColumns()) {
      Optional<RessourceGroup> matchingResourceGroup = ressourceGroups.stream()
                                                                      .filter(ressourceGroup -> ressourceGroup.getName()
                                                                                                              .equals(
                                                                                                                  resourceGroupNameColumn.getKey()))
                                                                      .findFirst();
      matchingResourceGroup.ifPresent(ressourceGroup -> {
  ...
  private void fillTreeGrid() {
    ...
    for (final RessourceGroup ressourceGroup : ressourceGroups) {
      Grid.Column<AufgabeHashMap> resourceGroupColumn = treeGrid.addColumn(
          hashmap -> hashmap.get(ressourceGroup.getName()))
                                                                .setTextAlign(ColumnTextAlign.END);
      ;
      applyColumnHeaderAndKey(resourceGroupColumn, ressourceGroup.getName());
      ...
      if (planningUnitList == null || planningUnitList.isEmpty()) {
        for (final RessourceGroup spalte : ressourceGroups) {
          ...
          final Optional<PlanningUnitElement> matchingPlanningUnitElement = planningUnitElementList.stream()
                                                                                                   .filter(
                                                                                                       planningUnitElement -> planningUnitElement.getRessourceGroup()
                                                                                                                                                 .getName()
                                                                                                                                                 .equals(
                                                                                                                                                     spalte.getName()))
                                                                                                   .findFirst();
...
//                        treeGrid.getTreeData().addItem(null, treeGridRowBean);
  ...
  private void addFootersToTreeGrid(CostsCalculator costsCalculator) {
    ...
    for (final RessourceGroup ressourceGroup : werteMap.keySet()) {
      ...
      treeGrid.getColumnByKey(ressourceGroup.getName())
              .setFooter(ressourceGroupKostenString);
    }
  }
...
//        datumField.setDateFormat(DATE_FORMAT.toPattern());
  ...
  private void fillFields() {
    ...
    summeField.setValue(timesCalculator.getGesamtSummeItem()
                                       .toString());
  ...
  public void setComponents() {
    overviewGrid.getElement()
                .getStyle()
                .set("margin-top", "10px");
    treeGrid.getElement()
            .getStyle()
            .set("margin-top", "10px");
    treeGrid.expandRecursively(treeGrid.getTreeData()
                                       .getRootItems()
                                       .stream(), 100);
    add(felderLayout);
  ...
  private void addiereZeileZurRessourceMap(final PlanningUnit planningUnit) {
    ...
    for (final PlanningUnitElement planningUnitElement : planningUnitElementList) {
      ...
      if (!ressourceGroup.getName()
                         .equals(messagesBundle.getString("aufgabe"))) {
