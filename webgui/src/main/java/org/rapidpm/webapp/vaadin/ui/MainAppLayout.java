...
public class MainAppLayout
  ...
  public MainAppLayout() {
    ...
    iconElement.getElement()
               .addEventListener("click", event -> UI.getCurrent()
                                                     .navigate(""));
    AppLayout build = AppLayoutBuilder.get(Behaviour.LEFT_RESPONSIVE)
                                      .withTitle("RapidPM 2019 - Vaadin 13 (" + VERSION + ")")
                                      .withAppBar(AppBarBuilder.get()
                                                               .add(new AppBarNotificationButton(VaadinIcon.BELL,
                                                                                                 notifications))
                                                               .build())
                                      .withAppMenu(LeftAppMenuBuilder.get()
                                                                     .addToSection(iconElement, HEADER)
                                                                     .add(LeftSubMenuBuilder.get(
                                                                         messages.getString("masterdata"),
                                                                         VaadinIcon.PLUS.create())
                                                                                            .add(
                                                                                                new LeftNavigationComponent(
                                                                                                    StundensaetzeScreen.class))
                                                                                            .add(
                                                                                                new LeftNavigationComponent(
                                                                                                    BenutzerScreen.class))
                                                                                            .build())
                                                                     .add(LeftSubMenuBuilder.get(
                                                                         messages.getString("projectmanagement"),
                                                                         VaadinIcon.PLUS.create())
                                                                                            .add(
                                                                                                new LeftNavigationComponent(
                                                                                                    ProjektplanungScreen.class))
                                                                                            .add(
                                                                                                new LeftNavigationComponent(
                                                                                                    AufwandProjInitScreen.class))
                                                                                            .add(
                                                                                                new LeftNavigationComponent(
                                                                                                    CostsScreen.class))
                                                                                            .add(
                                                                                                new LeftNavigationComponent(
                                                                                                    VertriebScreen.class))
                                                                                            .add(
                                                                                                new LeftNavigationComponent(
                                                                                                    OfferScreen.class))
                                                                                            .add(
                                                                                                new LeftNavigationComponent(
                                                                                                    ProjectAdministrationScreen.class))
                                                                                            .build())
                                                                     .addToSection(new LeftNavigationComponent(
                                                                         ImprintScreen.class), FOOTER)
                                                                     .build())
                                      .build();
    init(build);
  }
  ...
  private void prepare() {
    locale         = VaadinSession.getCurrent()
                                  .getAttribute(Locale.class);
    messages       = VaadinSession.getCurrent()
                                  .getAttribute(ResourceBundle.class);
    currentUser    = VaadinSession.getCurrent()
                                  .getAttribute(Benutzer.class);
    currentProject = VaadinSession.getCurrent()
