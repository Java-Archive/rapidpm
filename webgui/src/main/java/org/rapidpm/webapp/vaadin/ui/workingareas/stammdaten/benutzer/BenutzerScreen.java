...
//        benutzerGrid.setConverter("validUntil", new FormattedDateStringToDateConverter(DATE_FORMAT));
    ...
    removeButton.addClickListener((ComponentEventListener<ClickEvent<Button>>) buttonClickEvent -> {
      final Benutzer currentUser = VaadinSession.getCurrent()
                                                .getAttribute(Benutzer.class);
      final Benutzer selectedBenutzer = benutzerGrid.getSelectedItems()
                                                    .iterator()
                                                    .next();

      final boolean isUserDeletingHimself = currentUser.getLogin()
                                                       .equals(selectedBenutzer.getLogin());

      if (!isUserDeletingHimself) {
        final DaoFactory daoFactory12 = DaoFactorySingelton.getInstance();
        final List<Benutzer> benutzerFromDB = daoFactory12.getBenutzerDAO()
                                                          .loadBenutzerForLogin(selectedBenutzer.getLogin());
        if (benutzerFromDB != null && !benutzerFromDB.isEmpty()) {
          try {
            daoFactory12.removeTX(selectedBenutzer);
            benutzerGrid.setItems(benutzer.stream()
                                          .filter(einBenutzer -> einBenutzer.getId()
                                                                            .equals(selectedBenutzer.getId())));
...
//          logger.warn(selectedBenutzer.toString() + "war nur transient vorhanden");
    ...
    mandantenBox.addValueChangeListener(
        (HasValue.ValueChangeListener<AbstractField.ComponentValueChangeEvent<ComboBox<Mandantengruppe>, Mandantengruppe>>) comboBoxMandantengruppeComponentValueChangeEvent -> {
