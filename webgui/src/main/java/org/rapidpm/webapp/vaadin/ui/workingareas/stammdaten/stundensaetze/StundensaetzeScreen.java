...
public class StundensaetzeScreen
    extends Screen {
...
//    private StundensaetzeScreenBean screenBean;
  ...
  public StundensaetzeScreen() {
    try {
      ...
      final List<PlannedProject> plannedProjects = daoFactory.getPlannedProjectDAO()
                                                             .loadAllEntities();
      resourceGroups = daoFactory.getRessourceGroupDAO()
                                 .loadAllEntities();
      ...
      externerTagessatzPanel.addClickListener((ComponentEventListener<ClickEvent<Div>>) divClickEvent -> {
        externerTagessatzLayout.setFieldGroup(
            new ProjektFieldGroup(externerTagessatzLayout.getCurrentProject(), messagesBundle));
        externerTagessatzLayout.setExternalDailyRateField((TextField) externerTagessatzLayout.getFieldGroup()
                                                                                             .getFieldForProperty(
                                                                                                 PlannedProject.EXTERNALDAILYRATE)
        ...
        externerTagessatzLayout.getExternalDailyRateField()
                               .focus();
      });
      ...
      delRowButton.addClickListener((ComponentEventListener<ClickEvent<Button>>) buttonClickEvent -> {
        final RessourceGroup ressourceGroup = grid.getSelectedItems()
                                                  .iterator()
                                                  .next();
        try {
  ...
  private void handleEditMode() {
    ...
    switch (editMode) {
      case NOEDIT:
        gridListeners.add(grid.addItemClickListener(event -> delRowButton.setEnabled(!grid.getSelectedItems()
                                                                                          .isEmpty())));
        grid.getEditor()
            .cancel();
        break;
      case ROWEDIT:
        gridListeners.add(grid.addItemClickListener(event -> grid.getEditor()
  ...
  private void buildGrid() {
    ...
    final ResourceBundle messages         = VaadinSession.getCurrent()
    ...
    Grid.Column<RessourceGroup> column = grid.addColumn(RessourceGroup::getName)
                                             .setFlexGrow(5)
                                             .setFooter(GESAMTSUMMEN);
    applyColumnHeaderAndKey(column, NAME);
    column = grid.addColumn(new NumberRenderer<>(RessourceGroup::getBruttoGehalt, currencyInstance))
    ...
    column = grid.addColumn(
        new NumberRenderer<>(RessourceGroup::getFacturizable, NumberFormat.getPercentInstance(locale)))
                 .setFlexGrow(1);
  ...
  private void buildAndBindField(String resourceGroupProperty) {
    ...
    switch (resourceGroupProperty) {
      case NAME:
        field.getElement()
             .addEventListener("keydown", event -> grid.getEditor()
                                                       .cancel())
      ...
      case PLAN_ANZAHL:
        binder.forField(field)
              .withConverter(new StringToIntegerConverter("Integer only"))
              .bind(resourceGroupProperty);
        break;
      case FACTURIZABLE:
...
//                field.setPreventInvalidInput(true);
        binder.forField(field)
              .withConverter(Double::parseDouble, Object::toString)
              .bind(resourceGroupProperty);
        break;
      default:
        return;
  ...
  private void fillFooters() {
    ...
    grid.getColumnByKey(SUM_PER_MONTH)
        .setFooter(format.format(totalsByColumName.get(SUM_PER_MONTH)) + EUR);
    grid.getColumnByKey(SUM_PER_DAY)
