...
public class MappingPlanningUnitToIssueBase {
    ...
    public MappingPlanningUnitToIssueBase(final PlannedProject project) {
        ...
        dao = DaoFactorySingelton.getInstance().getIssueBaseDAO();
    }
...
//        issue.setReporter(project.getResponsiblePerson());
        ...
        IssueStoryPoint exist = storyPointDAO.findByName(stp.name(), project.getId());
        if (exist != null) {
        ...
        issue.setPriority(DaoFactorySingelton.getInstance().getIssuePriorityDAO().loadAllEntities(project.getId()).get(0));
        issue.setStatus(DaoFactorySingelton.getInstance().getIssueStatusDAO().loadAllEntities(project.getId()).get(0));
        issue.setType(DaoFactorySingelton.getInstance().getIssueTypeDAO().loadAllEntities(project.getId()).get(0));
        issue.setVersion(DaoFactorySingelton.getInstance().getIssueVersionDAO().loadAllEntities(project.getId()).get(0));
