...
public class DAO<K extends Number, E> implements Serializable {
    ...
    public E findByID(final String rid) {
        try {
            final Vertex vertex = orientDB.getVertexByKey("@rid", rid);
            Class<?> clazz = Class.forName(entityClass.getSimpleName());
            Constructor<?> constructor = clazz.getConstructor(Vertex.class);
            return (E) constructor.newInstance(vertex);
    ...
    public List<E> findAll(){
        try {
            ...
            for (final Vertex entityVertex : entityVertices) {
                ...
                Constructor<?> constructor = clazz.getConstructor(Vertex.class);
                E entity = (E) constructor.newInstance(entityVertex);
                entities.add(entity);
            }
            return entities;
        } catch (ClassNotFoundException | IllegalAccessException | InstantiationException | InvocationTargetException | NoSuchMethodException e) {
